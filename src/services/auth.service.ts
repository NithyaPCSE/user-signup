...
class AuthService extends Repository<UserEntity> {
  public async signup(userData): Promise<User> {
    ...
    const createUserData: User = await UserEntity.create({ email:userData.email, password: hashedPassword,username:userData.username,userSource:userData.userSource }).save();
